...
def find_flatten_cards(img, set_of_corners, debug=False):
    standard_width, standard_height = 200, 300
    img_outputs = []
    ...
    for i, corners in enumerate(set_of_corners):
        ...
        if maxWidth > maxHeight:
            dst_width, dst_height = standard_height, standard_width  # invertido
            rotate90 = True
        else:
            dst_width, dst_height = standard_width, standard_height
            rotate90 = False
        ...
        # Se estiver deitada, roda 90º para ficar vertical
        if rotate90:
        ...
        if debug:
            cv2.imshow(f'flat_{i}', img_output)
