...
import com.example.Jogo_da_Sueca.models.JoinGameRequest
import com.example.Jogo_da_Sueca.models.AddBotRequest
import com.example.Jogo_da_Sueca.models.RemoveParticipantRequest
...
class RoomActivity : AppCompatActivity() {
    ...
    private fun wireBotActions() {
        ...
        btnAddAgent1Bot.setOnClickListener {
            // sueca_1.3 backend currently only exposes random bots via BotFactory.
        ...
        lifecycleScope.launch {
            try {
                ...
                val response = RetrofitClient.api.addBot(
                    buildAddBotRequest(position, difficulty, botName)
                )

                if (response.success) {
                    val looksLikePendingJoin = response.playerId.isNullOrBlank() &&
                        (response.message?.contains("joining", ignoreCase = true) == true)

                    if (looksLikePendingJoin) {
                        ensureBotJoinedFromFrontend(position, botName)
                    } else {
                    ...
                    if (shouldFallbackToRandom(difficulty, response.message)) {
                        addBotWithRandomFallback(position, botName)
                    } else {
                ...
                if (shouldFallbackToRandom(difficulty, apiMessage)) {
                    val botName = "${pendingBotNamePrefix ?: "Bot"}_${normalizedPosition}_${(100..999).random()}"
                    addBotWithRandomFallback(position, botName)
                } else {
                    Toast.makeText(
                        this@RoomActivity,
                        apiMessage ?: "Erro ao adicionar bot.",
                        Toast.LENGTH_SHORT
                    ).show()
                }
            } catch (e: Exception) {
                Toast.makeText(this@RoomActivity, e.message ?: "Erro ao adicionar bot.", Toast.LENGTH_SHORT).show()
            }
        }
    }

    private suspend fun addBotWithRandomFallback(position: String, botName: String) {
        try {
            val fallback = RetrofitClient.api.addBot(
                buildAddBotRequest(position, "random", botName)
            )

            if (fallback.success) {
                val looksLikePendingJoin = fallback.playerId.isNullOrBlank() &&
                    (fallback.message?.contains("joining", ignoreCase = true) == true)
                ...
                if (looksLikePendingJoin) {
                    ...
                    Toast.makeText(
                        this@RoomActivity,
                        "Backend so suporta bot random. Bot adicionado como random.",
                        Toast.LENGTH_SHORT
                    ).show()
                    exitBotPlacementMode()
                    val state = RetrofitClient.api.getStatus(roomId)
                ...
                Toast.makeText(
                    this@RoomActivity,
                    fallback.message ?: "Nao foi possivel adicionar bot.",
                    Toast.LENGTH_SHORT
                ).show()
            }
        } catch (e: HttpException) {
            Toast.makeText(
                this@RoomActivity,
                extractApiErrorMessage(e) ?: "Erro ao adicionar bot.",
                Toast.LENGTH_SHORT
            ).show()
        } catch (e: Exception) {
            Toast.makeText(this@RoomActivity, e.message ?: "Erro ao adicionar bot.", Toast.LENGTH_SHORT).show()
    ...
    private fun buildAddBotRequest(position: String, difficulty: String, botName: String): AddBotRequest {
        return AddBotRequest(
            playerId = playerId,
            gameId = roomId,
    ...
    private suspend fun ensureBotJoinedFromFrontend(position: String, botName: String) {
        try {
            val joinResponse = RetrofitClient.api.joinGameWithPosition(
                JoinGameRequest(
                    name = botName,
                    gameId = roomId,
                    position = position
            ...
            if (joinResponse.success) {
                Toast.makeText(
                    this@RoomActivity,
                    "Bot adicionado (fallback de join no frontend).",
                    Toast.LENGTH_SHORT
                ).show()
            } else {
                // If backend added it asynchronously, this may return position taken.
                Toast.makeText(
                    this@RoomActivity,
                    joinResponse.message ?: "Bot em processo de entrada.",
                    Toast.LENGTH_SHORT
                ).show()
            }
        } catch (e: HttpException) {
            Toast.makeText(
                this@RoomActivity,
                extractApiErrorMessage(e) ?: "Falha ao confirmar entrada do bot.",
                Toast.LENGTH_SHORT
            ).show()
        } catch (e: Exception) {
            Toast.makeText(
                this@RoomActivity,
                e.message ?: "Falha ao confirmar entrada do bot.",
                Toast.LENGTH_SHORT
            ).show()
        } finally {
            exitBotPlacementMode()
            try {
                ...
                // Polling loop will refresh shortly.
            }
        }
    }

    private fun shouldFallbackToRandom(difficulty: String, apiMessage: String?): Boolean {
        if (difficulty.equals("random", ignoreCase = true)) return false
        return apiMessage?.contains("Unknown bot type", ignoreCase = true) == true
    }

    private fun extractApiErrorMessage(exception: HttpException): String? {
        return try {
            val raw = exception.response()?.errorBody()?.string().orEmpty()
            if (raw.isBlank()) return exception.message()
            JSONObject(raw).optString("message", exception.message())
        } catch (_: Exception) {
            exception.message()
        }
    }
