"""Decision maker for AverageAgent (hybrid engine)."""
...
class DecisionMaker:
    ...
    def choose_lead_card(self, legal_plays):
        ...
        partner_safe_non_trumps = [
            c for c in safe_non_trumps
            if not self.state.is_player_void(self.state.partner_id, CardMapper.get_card_suit(c))
        ]
            ...
            if winning_card:
                if CardMapper.get_card_rank(winning_card) in ["A", "7"]:
                    non_key_wins = [
                        ...
                        if c != winning_card
                        and CardAnalyzer.can_win_trick(
                            c,
                            self.state.current_trick,
                            self.state.trump_suit,
                            self.state.lead_suit,
