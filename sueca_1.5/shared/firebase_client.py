...
def get_player_game_history(player_id: str) -> list[dict]:
    ...
    docs = (
        ...
        .order_by("__name__")
        .start_at({"__name__": _DB.collection("player_game_history").document(prefix)})
        .end_before({"__name__": _DB.collection("player_game_history").document(prefix_end)})
        .stream()
    )
    ...
    for doc in docs:
        data = doc.to_dict() or {}
        entry = {k: v for k, v in data.items() if k not in excluded_fields}
        entry["doc_id"] = doc.id
