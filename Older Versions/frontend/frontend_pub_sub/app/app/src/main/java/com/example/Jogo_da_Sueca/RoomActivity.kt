...
import com.example.Jogo_da_Sueca.network.GameMqttSubscriber
...
class RoomActivity : AppCompatActivity() {
    ...
    private var hasNavigatedToGame: Boolean = false
    private var latestRealtimeState: GameStatusResponse? = null
    ...
    private fun startRealtimeUpdates() {
        // Initial snapshot keeps UI responsive even before first retained MQTT message arrives.
        refreshRoomStateSnapshot()

        mqttSubscriber?.disconnect()
        mqttSubscriber = GameMqttSubscriber(
            ...
            subscriber.connectAndSubscribe(
                ...
                onEnvelope = { envelope ->
                    runOnUiThread {
                        ...
                        if (state != null) {
                            ...
                            // Keep strict runtime MQTT behavior after joining.
                            if (playerId.isBlank()) {
                                // Pre-join bootstrap can still use one HTTP snapshot.
                                refreshRoomStateSnapshot()
                            }
                        }
                    }
                ...
                onConnectionError = {
                    // Keep silent to avoid toast spam during auto-reconnect cycles.
                }
            )
        }
    }

    private fun processIncomingState(state: GameStatusResponse) {
    ...
    private fun refreshRoomStateSnapshot() {
        lifecycleScope.launch {
            try {
                ...
                // Ignore transient HTTP failures; MQTT can still recover state.
            }
        }
    }

    private fun ensureStateHasCurrentPlayer() {
        lifecycleScope.launch {
            repeat(10) {
                val state = latestRealtimeState
                val hasMe = state?.players?.any {
                    (!playerId.isBlank() && it.id == playerId) || it.name == playerName
                } == true
                if (hasMe) {
                    return@launch
                }
            ...
            Toast.makeText(
                this@RoomActivity,
                "MQTT state not received yet. Aguarda ou reentra na sala.",
                Toast.LENGTH_SHORT
            ).show()
        }
    }
