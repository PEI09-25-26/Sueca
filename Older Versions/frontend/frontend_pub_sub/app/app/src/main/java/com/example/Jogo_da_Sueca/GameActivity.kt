...
class GameActivity : AppCompatActivity() {
    ...
    private fun startRealtimeUpdates() {
        // Initial snapshot prevents blank UI before first MQTT retained packet arrives.
        lifecycleScope.launch {
            try {
                GatewayClient.setRoomMode(gameId)
                val state = GatewayClient.getStatus(gameId.ifBlank { null })
                if (state != null) {
        ...
        mqttSubscriber = GameMqttSubscriber(
            ...
            subscriber.connectAndSubscribe(
                gameId = gameId,
                onEnvelope = { envelope ->
                    val state = envelope.state ?: return@connectAndSubscribe
                    runOnUiThread {
                        processIncomingState(state)

                        val currentPlayerId = playerId
                        if (currentPlayerId.isNotBlank()) {
                            val hand = envelope.hands[currentPlayerId]
                            if (hand != null) {
                                myHand = hand
                                updateHandUI()
                            }
    ...
    private fun playCard(card: Card) {
        val myPos = currentState?.players
            ?.find { (playerId.isNotBlank() && it.id == playerId) || it.name == playerName }
            ?.position
    ...
    private fun displayCachedCardsAndAnimate() {
        ...
        }, 700)  // Keep trick visible a bit longer so the fourth card is always seen.
    }
