...
class GameActivity : AppCompatActivity() {
    ...
    private var lastAnimatedRoundNumber = 0
    private var lastBotActionToken: String? = null
    private var lastBotActionAtMs: Long = 0L
        ...
        pollingJob = lifecycleScope.launch {
            while (true) {
                try {
                    fetchGameState()
                    fetchHand()
                    val delayMs = if (currentState?.phase == "playing") 500L else 1000L
                    delay(delayMs)
                } catch (e: Exception) {
    ...
    private suspend fun fetchGameState() {
        try {
            ...
            currentState = state
            updateUI(state)
            maybeDriveBotTurn(state)
        } catch (e: Exception) {
            e.printStackTrace()
        }
    }

    private suspend fun maybeDriveBotTurn(state: GameStatusResponse) {
        val now = System.currentTimeMillis()

        when (state.phase) {
            "deck_cutting" -> {
                val botName = state.northPlayer
                val botId = state.northPlayerId
                if (!isBotPlayer(botName) || botId.isNullOrBlank()) return

                val token = "deck_cutting:$botId:${state.currentRound}"
                if (!shouldRunBotAction(token, now)) return

                val cutIndex = Random.nextInt(1, 41)
                val res = RetrofitClient.api.cutDeck(
                    CutDeckRequest(playerId = botId, index = cutIndex, gameId = gameId.ifBlank { null })
                )
                if (res.success) markBotAction(token, now)
            }

            "trump_selection" -> {
                val botName = state.westPlayer
                val botId = state.westPlayerId
                if (!isBotPlayer(botName) || botId.isNullOrBlank()) return

                val token = "trump_selection:$botId:${state.currentRound}"
                if (!shouldRunBotAction(token, now)) return

                val choice = if (Random.nextBoolean()) "top" else "bottom"
                val res = RetrofitClient.api.selectTrump(
                    SelectTrumpRequest(playerId = botId, choice = choice, gameId = gameId.ifBlank { null })
                )
                if (res.success) markBotAction(token, now)
            }

            "playing" -> {
                val botName = state.currentPlayer
                val botId = state.currentPlayerId
                if (!isBotPlayer(botName) || botId.isNullOrBlank()) return

                val token = "playing:$botId:${state.currentRound}:${state.roundPlays.size}"
                if (!shouldRunBotAction(token, now)) return

                val handResponse = RetrofitClient.api.getHand(botId, gameId.ifBlank { null })
                if (!handResponse.success || handResponse.hand.isEmpty()) return

                val selectedCard = chooseBotCard(handResponse.hand, state.roundSuit)
                val res = RetrofitClient.api.playCard(
                    PlayRequest(playerId = botId, card = selectedCard, gameId = gameId.ifBlank { null })
                )
                if (res.success) markBotAction(token, now)
            }
        }
    ...
    private fun chooseBotCard(hand: List<String>, roundSuit: String?): String {
        if (roundSuit.isNullOrBlank()) return hand.random()

        val matchingSuit = hand.filter { cardId ->
            val numeric = cardId.toIntOrNull() ?: return@filter false
            CardMapper.getCardSuit(numeric).equals(roundSuit, ignoreCase = true)
        }

        return if (matchingSuit.isNotEmpty()) matchingSuit.random() else hand.random()
    }

    private fun isBotPlayer(name: String?): Boolean {
        if (name.isNullOrBlank()) return false
        return name.startsWith("RandomBot", ignoreCase = true) ||
            name.startsWith("Agent1Bot", ignoreCase = true) ||
            name.contains("bot", ignoreCase = true)
    }

    private fun shouldRunBotAction(token: String, nowMs: Long): Boolean {
        if (token != lastBotActionToken) return true
        return nowMs - lastBotActionAtMs > 1800L
    }

    private fun markBotAction(token: String, nowMs: Long) {
        lastBotActionToken = token
        lastBotActionAtMs = nowMs
    }
    ...
    private fun updateTableCards(roundPlays: List<RoundPlay>) {
        ...
        // Animate collection only after all 4 cards are visible on the table.
        if (roundPlays.size == 4 && currentRound > lastAnimatedRoundNumber) {
            lastAnimatedRoundNumber = currentRound
